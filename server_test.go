...
var _ = Describe("Server", func() {
	...
	It("setups with the right values", func() {
		...
		vsa := func(_ net.Addr, _ *STK) bool { return true }
		config := Config{
			TLSConfig:           &tls.Config{},
		...
		Expect(reflect.ValueOf(server.config.VerifySourceAddress)).To(Equal(reflect.ValueOf(vsa)))
	})
	...
	It("fills in default values if options are not set in the Config", func() {
		...
		Expect(reflect.ValueOf(server.config.VerifySourceAddress)).To(Equal(reflect.ValueOf(defaultSourceAddressVerification)))
	})
...
var _ = Describe("default source address verification", func() {
	It("accepts a token", func() {
		...
		Expect(defaultSourceAddressVerification(remoteAddr, stk)).To(BeFalse())
	})

	It("requests verification if no token is provided", func() {
		remoteAddr := &net.UDPAddr{IP: net.IPv4(192, 168, 0, 1)}
		Expect(defaultSourceAddressVerification(remoteAddr, nil)).To(BeTrue())
	})
	...
	It("rejects a token if the address doesn't match", func() {
		...
		Expect(defaultSourceAddressVerification(remoteAddr, stk)).To(BeTrue())
	})
